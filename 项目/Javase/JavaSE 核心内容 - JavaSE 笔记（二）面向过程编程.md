...
1. **GB2312 / GBK**：解决中文，规则是"字节>127判断汉字开始"，GBK 更宽松
2. **Unicode 是字符集，不是编码**：只管"有哪些字符"，不管"怎么存"
3. **UTF-8 自适应变长**：英文1字节、中文3字节、生僻字4字节，省空间且兼容ASCII
4. **Java 运行时用 UTF-16**：对中文更友好，几乎覆盖全球字符
5. **ASCII 码**是这一切的起点，UTF-8 完美兼容它，所以英文几乎不吃亏

一句话：**Unicode 解决"能表示什么"，UTF-8/UTF-16 解决"怎么高效存储"**。两者不能搞混。
...
> `yield`是 Java 14 随 Switch 表达式引入的一个**上下文关键字**（不是完整保留字），只用在 switch 表达式的 `case` 分支中，用来**指定该分支的返回值**。它的存在是因为：当分支里只有简单返回值时，箭头 `->` 后面直接写值即可；但一旦用 `{}` 包裹多行代码（比如有前置操作），编译器就不知道这一坨代码最终要返回什么了——这时就需要 `yield` 来"交卷"。
>
> **三种情况对比：**
>
> * 简单单行：`case 9 -> 'A';`  —— 箭头后面直接给值
> * 多行代码块：`case 7 -> { doSomething(); yield 'C'; }`  —— 花括号里最后用 `yield`
> * 传统冒号写法（Java 21 前不可混用）仍用 `break`
>
> **`yield` 的规则：**
>
> | 规则 | 说明 |
> |------|------|
> | 只能在 switch **表达式**中用 | 有返回值的 switch（被变量接收的那种），传统 switch **语句**里不能写 |
> | 必须放在代码块最后 | `yield` 执行后分支结束，后面不能再有任何代码 |
> | 不可混用语法（Java 21 前） | 箭头语法 (`->`) 和传统冒号语法 (`:`) 不能混在一起 |
>
> 注意 `yield` 和 `return` 不同—— `return` 是跳出整个方法，而 `yield` 只是跳出当前 switch 分支，方法里的后续代码正常执行。
...
原文链接：https://www.itbaima.cn/zh-CN/document/pew6po6wrou23pk3?segment=1#%E7%A8%8B%E5%BA%8F%E4%BB%A3%E7%A0%81%E5%9F%BA%E6%9C%AC%E7%BB%93%E6%9E%84