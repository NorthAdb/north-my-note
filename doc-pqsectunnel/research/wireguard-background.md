...
- 首字母 `I`：initiator 静态公钥 **立即** 发送给 responder（identity hiding 有所降低）
...
| ECDH                         | 依赖什么私钥？                 | 绑定类型  | 绑谁               |
| ---------------------------- | ----------------------- | ----- | ---------------- |
| `es = DH(e_i^priv, S_r^pub)` | e_i^priv + S_r^priv     | A     | responder 静态（单向） |
| `ss = DH(S_i^priv, S_r^pub)` | **S_i^priv + S_r^priv** | **B** | **双方静态互锁**       |
| `ee = DH(e_r^priv, e_i^pub)` | e_r^priv + e_i^priv     | B     | 双方临时互锁（本次会话）     |
| `se = DH(e_r^priv, S_i^pub)` | e_r^priv + S_i^priv     | A     | initiator 静态（单向） |
